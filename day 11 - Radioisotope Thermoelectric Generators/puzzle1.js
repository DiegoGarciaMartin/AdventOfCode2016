...
function calculateMinimumNumberSteps(stateIni){
	...
	while (!problemResolved && states[steps].length > 0){
		...
		while (i < states[steps - 1].length && !problemResolved){
			...
			removePossibleStepsDuplicates2(statesMaps, possibleSteps);
...
var stateExample = {'floor1': ['HyM', 'LiM'],
			  	 	'floor2': ['HyG'],
			  	 	'floor3': ['LiG'],
			  	 	'floor4': [],
			  	 	'elevator': 1,
...
//console.log('Minimum number steps example -> ' + calculateMinimumNumberSteps(stateExample) + '\n\n');

console.log('Minimum number steps -> ' + calculateMinimumNumberSteps(processInput(input)) + '\n\n');